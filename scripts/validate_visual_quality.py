        ...
        for model in diagram.iter("mxGraphModel"):
            ...
            for cell in model.findall(".//mxCell"):
                ...
                if edge_attr == "1" or "edge" in props:
                    ...
                    # Explicit Draw.io containers and large dashed grouping boxes are
                    # layout containers.  Their children/inside edges should not be
                    # reported as collisions with the container background itself.
                    v.is_container = bool(
                        props.get("container") == "1"
                        or (
                            v.dashed
...
def check_spacing_variance(vertices: List[Vertex]) -> List[Finding]:
    ...
    for key, indices in y_clusters.items():
        ...
        for i in range(len(row_items) - 1):
            # Only compare boxes that genuinely occupy the same visual row.
            # Nearby stacked grid rows can have top coordinates within the
            # clustering tolerance without sharing any vertical extent.
            if min(row_items[i].rect.y2, row_items[i + 1].rect.y2) <= max(row_items[i].rect.y, row_items[i + 1].rect.y):
                continue
        ...
        # Two pixels is deliberate and readable for compact token rows and
        # attention matrices; reserve FAIL for actual contact/near-contact.
    ...
    for key, indices in x_clusters.items():
        ...
        for i in range(len(col_items) - 1):
            # Likewise, compare only boxes that genuinely share a column;
            # adjacent matrix columns must not be treated as overlaps.
            if min(col_items[i].rect.x2, col_items[i + 1].rect.x2) <= max(col_items[i].rect.x, col_items[i + 1].rect.x):
                continue
